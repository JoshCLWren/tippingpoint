...
import { useMileState } from '../TripCalculator/MileContext';
      ...
      return isAuthenticated && (
        <div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <label>GPS Coordinates should be entered LONGITUDE,LATTITUDE example: -95.760074,36.062184</label>
              <input
                name="gps"
                ref={register({ required: true, minLength: 7, pattern: { value: /^([-]?)([\d]{1,2})(((\.)(\d+)(,)))(([-]?)([\d]{1,3})((\.)(\d+))?)$/g} })}
                placeholder="Enter or search for GPS coordinates"
                value={searchResults}
                // onChange={onInputChange}
              />
